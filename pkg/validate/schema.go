...
type field struct {
	...
	// covers fields the spec and this library disagree on.
	note string
}
...
var (
	...
	statementFields = []field{
		{name: fieldID, kind: typeString},
		{
			...
			note: "the statement version field is defined by the OpenVEX spec but go-vex does not decode it; it is dropped when the document is rewritten",
		},
		...
		{
			name: fieldSupplier, kind: typeString,
			note: "the statement supplier field is defined by the OpenVEX spec but go-vex does not decode it; it is dropped when the document is rewritten",
		},
...
// matter because decoding drops them without a word, which is how a typo in a
