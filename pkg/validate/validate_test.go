...
func TestDocumentJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			requireFindings(t, Bytes([]byte(tc.doc)), tc.want)
		})
	}
}
...
func TestDocumentContext(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			want: []expected{{SeverityWarning, CheckContext, "@context"}},
		},
		{
		...
		},
		{
			...
			want: []expected{{SeverityError, CheckContext, "@context"}},
		},
		{
			name: "a context of the wrong type",
			doc:  `{"@context": 12}`,
			want: []expected{{SeverityError, CheckFieldType, "@context"}},
		},
		{
		...
		},
		{
			...
			want: []expected{
				{SeverityError, CheckRequired, "@context"},
				{SeverityWarning, CheckStatements, "statements"},
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestDocumentSchema(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			want: []expected{
				{SeverityError, CheckRequired, "@id"},
				{SeverityError, CheckRequired, "author"},
				{SeverityError, CheckRequired, "timestamp"},
				{SeverityError, CheckRequired, "version"},
				{SeverityError, CheckRequired, "statements"},
			},
		},
		{
		...
		},
		{
			...
			want: []expected{
				{SeverityError, CheckFieldType, "version"},
				{SeverityWarning, CheckJSON, ""},
			},
		},
		{
		...
		},
		{
			...
			want: []expected{
				{SeverityError, CheckFieldType, "timestamp"},
				{SeverityWarning, CheckJSON, ""},
			},
		},
		{
			...
			want: []expected{
				{SeverityError, CheckFieldType, "statements"},
				{SeverityWarning, CheckJSON, ""},
			},
		},
		{
		...
		{
			name: "spec fields go-vex drops are warned about",
...
func TestDocumentMetadata(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			want: []expected{{SeverityWarning, CheckIRI, "@id"}},
		},
		{
		...
		},
		{
			...
			want: []expected{{SeverityError, CheckVersion, "version"}},
		},
		{
...
func TestDocumentRepeatedStatementID(t *testing.T) {
  ...
  "statements": [
	...
	requireFindings(t, Bytes([]byte(doc)), []expected{
