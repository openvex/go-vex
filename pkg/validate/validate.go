...
// Package validate is a linter for OpenVEX documents, meant for documents
// written or edited by hand.
//
// It reports two kinds of findings. Errors mark documents that break the
// OpenVEX specification and that VEX consumers may reject or read incorrectly.
// Warnings mark data that parses but is suspicious: fields this library
...
// problem, so that a single pass tells the author everything that needs
// fixing.
//
// Validate a document on disk with [File], or one already in memory with
// [Bytes]:
//
//	res := validate.File("vex.json")
//	if !res.Valid() {
...
// Bytes validates an OpenVEX document held in memory.
func Bytes(data []byte) *Result {
...
// checks walk: it preserves the fields the vex structs would drop.
...
// to the document. They only do for the OpenVEX version this package implements.
func (r *Result) checkContext(raw map[string]any) bool {
	...
	if !found || value == nil {
		if looksLikeCSAF(raw) {
			r.errorf(
				CheckContext, "",
				"this is a CSAF document and validate only checks OpenVEX documents; vex.OpenCSAF reads CSAF data and returns it as OpenVEX",
			)
			return false
		}
		...
		// against the version this package implements.
		return true
	}
	...
	r.warnf(
		CheckContext, fieldContext,
		"the document declares OpenVEX %s but validate checks documents against v%s, so no further checks ran; vex.Open reads older documents in compatibility mode and returns them in the current version",
...
// decode parses the document into the vex structs, which the semantic
// checks read. Decoding errors the schema pass already explained are not
// repeated.
func (r *Result) decode(data []byte) (*vex.Document, bool) {
	doc := &vex.Document{}
...
func (r *Result) checkMetadata(doc *vex.Document, raw map[string]any) {
	if doc.ID != "" {
...
func (r *Result) checkStatements(doc *vex.Document) {
	seen := map[string]int{}
...
func (r *Result) checkStatement(stmt *vex.Statement, doc *vex.Document, path string) {
...
func (r *Result) checkStatementDates(stmt *vex.Statement, doc *vex.Document, path string) {
